		...
		if (veces == 0) {
			...
			for (int i = -1; i < 2; i++  ) {
				...
				for (int k = -1; k < 2; k++) {
					...
					if (coordY-k >= 0 && coordX-i >= 0 && coordY-k <= input[0].length-1 && coordX-i <= input[0][0].length-1) { // No se salga de matriz
						
						if (!(input[1][coordY-k][coordX-i].contains("O")) && !(input[1][coordY-k][coordX-i].contains("X")) && veces < 2) { 
